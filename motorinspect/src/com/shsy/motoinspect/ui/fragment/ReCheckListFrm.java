...
public class ReCheckListFrm extends BaseFragment {
...
//		showSearchDialog();
	...
	private String getUrl(){
		return ToolUtils.getCheckedList(mActivity);
	}
	...
	private void getCarListNetwork(String url,final String hphm) {
		...
		builder.build().execute(new ListCarInfoCallback(){
	...
	private void viewSetAdapter() {
		...
		mListView.setOnItemClickListener(new OnItemClickListener(){
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if(mCallback !=null){
					mCallback.onReCheckItemClick(mCarList.get(position));
				}
			}
		});
	}
	...
	private void searcheCar(String hphm){
		if(hphm.length()<2){
			ToastUtils.showToast(mActivity, "号牌至少输入2位", Toast.LENGTH_SHORT);
			return;
		}
	...
	public OnReCheckListClick mCallback;
	public interface OnReCheckListClick{
	...
	public void onAttach(Activity activity) {
		...
		try {
			mCallback = (OnReCheckListClick) mActivity;
		} catch (ClassCastException e) {
			e.printStackTrace();
			throw new ClassCastException(mActivity.toString()
					+ " must implement OnReCheckListClick");
		}
	}
}
